...
export default function RepliesPost(root, userId) {
    root.innerHTML = '';
    fetchRepliesPost(root,userId);
}

function fetchRepliesPost(root, userId) {
    axios.get(`http://localhost:3000/v1/account/profile/replies/${userId}`, {
        headers: {
            apikey: 'jenather',
...
function initializeEventListeners(postElement, post) {
    ...
    repostButton.addEventListener('click', () => {
        if (post.isReposted) {
            actionUnrepost(post.post_id, repostButton); // Call Unrepost
            post.isReposted = false;
        } else {
            actionRepost(post.post_id, repostButton); // Call Repost

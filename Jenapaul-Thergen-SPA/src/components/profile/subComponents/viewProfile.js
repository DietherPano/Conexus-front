...
import Follow from '../../activity/followFunction/follow';
import Unfollow from '../../activity/followFunction/unfollow';
...
export default function addViewProfileListener() {
    ...
    const profiles = document.querySelectorAll('.user-profile2');
...
function fetchAccountPost(root,userId) {
    root.innerHTML = ''
    axios.get(`http://localhost:3000/v1/account/post/${userId}`, {
        headers: {
            apikey: 'jenather',
            token: localStorage.getItem('token'),
        },
...
function initializeEventListeners(postElement, postData) {
    ...
    likesButton.addEventListener('click', () => { 
        if (postData.isLiked) {
            accountReactionUnlike(postElement, postData.post_id);
            postData.isLiked = false;
            likesButton.classList.remove('liked');
        } else {
            accountReactionLike(postElement, postData.post_id);
            postData.isLiked = true;
            likesButton.classList.add('liked');
        }
    });

    // Handle replies button
    repliesButton.addEventListener('click', () => {
        console.log(`Replies clicked for post ID: ${postData.post_id}`);
        // Implement replies functionality
    });

    // Handle repost button
    repostButton.addEventListener('click', () => {
        if (postData.isReposted) {
            actionUnrepost(postElement, postData.post_id);
            postData.isReposted = false;
            repostButton.classList.remove('reposted');
        } else {
            actionRepost(postElement, postData.post_id);
